					...
					struct v4l2_capability *cap)
{
	...
	if (go != NULL) strlcpy(cap->card, go->name, sizeof(cap->card));
#if 0
	...
	if ((go != NULL) && (go->board_info->flags & GO7007_BOARD_HAS_TUNER))
...
static long go7007_do_ioctl(struct file *file,
		...
		unsigned int cmd, unsigned long arg)
{
	...
	retval = video_ioctl2(file, cmd, arg);
        if (retval < 0) return video_usercopy(file, cmd, arg, (v4l2_kioctl)go7007_do_ioctl);
        return retval;
}
...
static struct v4l2_file_operations go7007_fops = {
	...
	.ioctl		= go7007_ioctl/*video_ioctl2*/,
	.read		= go7007_read,
	.mmap		= go7007_mmap,
	.poll		= go7007_poll,
};
